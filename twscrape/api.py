...
OP_FollowersYouKnow = "xEfTm022AXXAKtEamdUIDA/FollowersYouKnow"
...
class API:
    ...
    async def followers_you_know_raw(
        self,
        uid: int,
        limit=-1,
    ...
    async def followers_you_know(
        ...
        async with aclosing(
            self.followers_you_know_raw(uid, limit=limit, cursor=cursor, kv=kv)
        ) as gen:
            async for rep in gen:
                for x in parse_users(rep.json(), limit):
                    yield x
